...
// ── VOICE SELECTION ───────────────────────────────────────────────
// Tries to find a female Hebrew voice; falls back to high pitch to simulate one
let _chosenVoice = null;

function loadVoice() {
  if (_chosenVoice) return;
  const voices = speechSynthesis.getVoices();
  if (!voices.length) return;

  // Priority 1: Hebrew female voice
  _chosenVoice =
    voices.find(v => v.lang.startsWith('he') && /female|woman|girl|נשי/i.test(v.name)) ||
    // Priority 2: any Hebrew voice
    voices.find(v => v.lang.startsWith('he')) ||
    // Priority 3: any female voice (will still use he-IL lang tag)
...
if (window.speechSynthesis) {
  ...
  loadVoice(); // also try immediately
}
...
function speak(text) {
  ...
  u.lang  = 'he-IL';
  u.rate  = 0.78;
  u.pitch = _chosenVoice ? 1.5 : 1.9; // higher pitch if no female voice found
